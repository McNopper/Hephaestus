# Architecture â€” opencode-eclipse
...
1. **Eclipse vs. non-Eclipse (Java):** pure Java must stay usable *outside* Eclipse/OSGi â€” as a
...
2. **Java vs. non-Java:** the web renderer (HTML/JS/CSS + KaTeX/highlight.js/mermaid) is a
   standalone front-end component with a **versioned bridge contract** â€” reusable in any host
...
4. **Coding-agent backend (opencode today, others later):** all agent interaction goes through the
   ...
   NOT pre-build a generic abstraction â€” we keep the boundary clean so a `CodingAgent` port can be
...
## Module organization â€” the three tiers (verified against manifests/poms 2026-08-20)
...
`-DskipEclipseBan=true` skips it in client/git/fleet â€” the tools/tasks bans are not skippable) â€” usable
...
### Tier 1 â€” reusable plain Java (Eclipse-free, build-enforced)

| Module | Depends on | What it is |
|---|---|---|
| `com.opencode.ide.client` (+ tests) | gson | opencode REST/SSE client, DTOs, `ChatRequests`/`McpRequests`, `OpencodeEventStream`, server launcher; hardened error semantics; `RuntimeTuning` (live poll/stall/budget/worker knobs) + `WorkerPools` (one bounded shared worker pool â€” no thread per call) |
| `com.opencode.ide.tools` (+ tests) | gson | `ToolProvider` SPI + `McpDispatcher` (JSON-RPC) â€” language-agnostic; the C++ pack lives beside it (tier 3) |
...
### Tier 3 â€” C++-development specifics

| Module / package | What it is |
|---|---|
| `tools` bundle â†’ `tools.cpp` package | `CppToolProvider`: toolchain detection (MSVC via vswhere, MSYS2 envs), cmake configure/build, ctest, run, gdb-batch, clang-tidy/cppcheck, clang-format â€” behind the `ToolProvider` SPI; extracted into its own bundle when a second language pack lands |
...
- **M1 âœ… (2026-08-15):** `client` split out (69 tests moved, core.tests deleted); Eclipse-import
  ...
  `OpencodeEventStream` are public in client (core constructs them directly â€” no extra
  interfaces invented); only `HttpOpencodeClient`/`Auth` stay internal. Follow-up candidates:
  eclipse-ize only when needed â€” `ClientLog` adapter lives in core's activator.
...
- **M4 â€” deferred (unchanged):** when a second coding-agent backend is seriously evaluated,
...
## Live activity derivation (`client.activity` â€” ported from the retired opencode-viewer)
...
tracked invocation**), and the **active-files map** â€” a file (first non-blank of
...
view renders `thinkingâ€¦` / `tool: <name> â€” <file>` labels and the "Active files" node from it.
...
- **Host â†’ page** (via `ChatScripts`; payloads are JSON *string literals* â€” objects tolerated;
  every entry point is guarded, returns `true`/`false`, reports errors via
  `JS ERROR in <fn>: â€¦` so nothing fails silently the way `Browser.execute()` does):
  ...
  `[{name, state}]` rendering as compact `tool: name â€” state` lines),
  `__appendDelta({mid, text})` (raw text stream + cursor, no markdown),
  `__setMessages([â€¦])` (history, same per-entry shape incl. `tools`), `__clear()`,
  `__stopStream({mid})` (removes the streaming cursor when a send settles/fails/aborts â€”
  idempotent, keeps the streamed text).
  (`__linkClick(event)` exists but is the page's internal test-exposed click interceptor.)
- **Page â†’ host:** `__javaReport(type, detail)` â€” `page-ready` flush signal, render
  confirmations (`user/assistant bubble renderedâ€¦`, `history rendered (N entries)`,
  `mermaid initialised/diagram rendered`, `KaTeX failedâ€¦`), JS errors; and
...
- **Checks:** `components/chat-web` runs `renderer-check.mjs` (51) + `bridge-check.mjs` (94) +
  `mermaid-check.mjs` (8 â€” drives the real page in **headless Microsoft Edge** via
  `puppeteer-core`: diagrams must render to SVG, broken sources degrade visibly, no silent drops;
  SKIPs when Edge/puppeteer-core are absent) â€” all wired into the chat bundle's `mvn verify`
...
2. Did any language-specific logic (cmake/clang/msvc pathsâ€¦) escape a `ToolProvider`?
