...
| `com.opencode.ide.git` (+ tests) | — | `WorktreeManager` + `FleetGit` conventions (branch/worktree naming, `STORE_PATH`) over the git CLI; `StoreGitStatus`/`StoreSync` (distributed-fleet store status + commit→pull-rebase→push discipline, **path-scoped to the store subtree**) |
